    ...
    for (index_row,file_line) in file_list.clone().enumerate(){
        ...
        while let Some((index_char, file_item)) = chars_iter.next() {
            if is_digit(file_item){
                ...
                // Let's get all numbers around the symbol
                ...
                // println!("found number from index: {:?} till {:?} with number{:?}",index_char,index_number, numbers_around_symbol);

                // now we need to check if there is a symbol around the number series
                if let Some(_symbol_index) = find_symbol_around_numbers(&file_contents, index_char, index_number as usize,index_row) {
                    // let's add them to to totals
                    println!("yes symbol found at row: {:?} at chars{:?} till {:?} with number{:?}",index_row,index_char,index_number, numbers_around_symbol);

                    if let Ok(combined_integer) = numbers_around_symbol.parse::<u32>() {
                        total_sum += combined_integer;
                    } else {
                    ...
                    println!("no symbol found at row: {:?} at chars{:?} till {:?} with number{:?}",index_row,index_char,index_number, numbers_around_symbol);
                }
            }
        }
    }
