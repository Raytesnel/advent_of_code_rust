    ...
    for (index_row,file_line) in file_list.clone().enumerate(){
        ...
        while let Some((index_char, file_item)) = chars_iter.next() {
            if is_digit(file_item){
                let ( numbers_around_symbol, index_number) = get_all_number_series(index_char, file_item, &mut chars_iter);
                // now we need to check if there is a symbol around the number series
                match check_symbol_around(&file_contents, index_char, index_number as usize, index_row, numbers_around_symbol){
                    Ok(result)=>{
                        total_sum += result
                    }
                    Err(error)=>{
...
fn check_symbol_around(file_contents:&String, index_char:usize, index_number:usize, index_row:usize, numbers_around_symbol:String) ->Result<u32,String>{
    if let Some(_symbol_index) = find_symbol_around_numbers(&file_contents, index_char, index_number,index_row) {
...
fn get_all_number_series(index_char:usize,file_item:char,chars_iter:&mut Peekable<Enumerate<Chars<'_>>>)->(String,u32){
...
fn find_symbol_around_numbers(file_list: &str, start_index: usize, end_index: usize,index_row:usize) -> Option<usize> {
    ...
    for (_index_row, file_line) in file_list.lines().skip(start_row).take(end_row-start_row+1).enumerate() {
        for (index, symbol_candidate) in file_line.chars().skip(symbol_start).take(symbol_end - symbol_start+1).enumerate() {
